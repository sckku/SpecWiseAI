...
export function ExecutiveDashboard() {
  ...
  const pendingCount = proposals.filter((p) =>
    ["DEPT_REVIEW", "SUBMITTED", "AI_ANALYZED"].includes(p.status)
  ).length;
  const revisedCount = proposals.filter((p) => p.status === "REVISED").length;
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <form onSubmit={handleStartAnalysis} className="pt-2">
            <div className="bg-white rounded-2xl p-1.5 sm:p-2 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <input
                ...
                placeholder="เช่น ต้องการเครื่องคอมพิวเตอร์สำหรับวิจัย AI, กล้องจุลทรรศน์, จอ Smart Display..."
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          >
            <span>ดูรายการทั้งหมด ({proposals.length})</span>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Link
            href="/requests"
            className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs hover:shadow-md hover:border-blue-300 transition-all flex items-start space-x-3.5 group"
          >
            <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                งบรวม {(metrics ? (metrics.totalBudgetRequestedBaht / 1000000).toFixed(1) : (proposals.reduce((s, p) => s + p.totalBudgetBaht, 0) / 1000000).toFixed(1))} ลบ. <span className="text-emerald-600 font-semibold">(+14% เทียบกับปีที่แล้ว)</span>
              </p>
            </div>
          </Link>
          ...
          {/* 2. Pending Review */}
          <Link
            href="/requests?status=DEPT_REVIEW"
            className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs hover:shadow-md hover:border-amber-300 transition-all flex items-start space-x-3.5 group"
          >
            <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-500 group-hover:bg-amber-500 group-hover:text-white transition-colors flex items-center justify-center shrink-0">
              <Clock className="w-5 h-5" />
            </div>
            <div>
              <div className="text-2xl font-heading font-bold text-slate-900 leading-tight">
                {metrics?.pendingReviewCount ?? pendingCount}
              </div>
              <div className="text-xs font-semibold text-slate-700 mt-0.5">รอตรวจสอบ</div>
              <p className="text-[11px] text-amber-600 font-medium mt-0.5">
                อยู่ระหว่างการพิจารณา <span className="text-slate-400 font-normal">(-8% เทียบกับปีที่แล้ว)</span>
              </p>
            </div>
          </Link>
          ...
          {/* 3. Needs Revision */}
          <Link
            href="/requests?status=REVISED"
            className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs hover:shadow-md hover:border-rose-300 transition-all flex items-start space-x-3.5 group"
          >
            <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-500 group-hover:bg-rose-500 group-hover:text-white transition-colors flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-[11px] text-rose-600 font-medium mt-0.5">
                รอผู้ขอปรับแก้เอกสาร <span className="text-emerald-600 font-semibold">(-45% เทียบกับปีที่แล้ว)</span>
              </p>
            </div>
          </Link>
          ...
          <Link
            ...
            className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs hover:shadow-md hover:border-emerald-300 transition-all flex items-start space-x-3.5 group"
          >
            <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white transition-colors flex items-center justify-center shrink-0">
              <CheckCircle2 className="w-5 h-5" />
            </div>
            <div>
              <div className="text-2xl font-heading font-bold text-slate-900 leading-tight">
                {metrics?.approvedCount ?? approvedCount}
              </div>
              <div className="text-xs font-semibold text-slate-700 mt-0.5">อนุมัติบรรจุแผน</div>
              <p className="text-[11px] text-emerald-600 font-medium mt-0.5">
                พร้อมจัดซื้อจัดจ้าง <span className="text-emerald-700 font-bold">(+28% เทียบกับปีที่แล้ว)</span>
              </p>
            </div>
          </Link>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-2.5">
              {proposals
                .filter((p) => ["AI_ANALYZED", "DEPT_REVIEW", "REVISED"].includes(p.status))
                .slice(0, 4)
                .map((prop) => (
                  <Link
                    key={prop.id}
                    href={`/requests/${prop.id}`}
                    className="flex items-center justify-between gap-2 rounded-2xl border border-slate-100 p-3 transition-all hover:border-indigo-200 hover:bg-slate-50/70 group"
                  >
                    <div className="flex items-center space-x-3 min-w-0">
                      ...
                      >
                        {prop.category.includes("วิทย์") || prop.category.includes("แพทย์") ? (
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 truncate">
                          {prop.department} • {prop.requesterName}
                        </p>
                      </div>
                    </div>
            ...
            <div className="flex items-center justify-center gap-6 py-1">
              ...
              <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle
                    ...
                    strokeDashoffset="113.04" /* 55% */
                    strokeLinecap="round"
                    fill="none"
                  />
                </svg>
                <div className="absolute text-center">
                  <div className="text-lg font-heading font-bold text-slate-900">55%</div>
              ...
              <div className="space-y-1 text-xs flex-1 min-w-0">
                <div className="flex items-center space-x-2 text-slate-700">
                  ...
                  <span className="text-xs font-medium truncate">1. สกัดความต้องการ</span>
                </div>
                <div className="flex items-center space-x-2 text-slate-700">
                  <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 text-white flex items-center justify-center text-[9px] font-bold shrink-0">
                    ✓
                  </div>
                  <span className="text-xs font-medium truncate">2. รายการมาตรฐาน</span>
                </div>
                <div className="flex items-center space-x-2 text-slate-700">
                  <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 text-white flex items-center justify-center text-[9px] font-bold shrink-0">
                    ✓
                  </div>
                  <span className="text-xs font-medium truncate">3. ราคาอ้างอิง 4 ฐาน</span>
                </div>
                ...
                <div className="flex items-center space-x-2 text-slate-400">
                  ...
                  <span className="text-xs truncate">6. TOR เป็นกลาง</span>
                </div>
              </div>
            </div>

            <div className="mt-3 p-3 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex items-center justify-between text-xs">
              <span className="text-indigo-900 font-medium">ความพร้อมเฉลี่ยระดับ มข.:</span>
          ...
          <div>
            ...
            <div className="space-y-2.5">
              ...
              <div className="p-2.5 rounded-2xl border border-slate-100 hover:bg-slate-50 transition-colors flex items-start space-x-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="text-[11px] text-slate-500 truncate">AI GPU Workstation</span>
              ...
              <div className="p-2.5 rounded-2xl border border-slate-100 hover:bg-slate-50 transition-colors flex items-start space-x-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-bold text-slate-900 truncate">
                    แนบใบเสนอราคา 3 บริษัท
                  </h4>
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="text-[11px] text-slate-500 truncate">Smart Display 75 นิ้ว</span>

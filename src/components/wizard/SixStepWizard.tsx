  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-card space-y-4">
        ...
        <div className="relative">
          ...
          <div className="flex justify-between items-center mt-2">
            <div className="flex flex-wrap gap-1.5">
              {QUICK_PROMPTS.map((qp, idx) => (
                <button
                  ...
                  className="text-sm px-2.5 py-1 rounded-lg border border-slate-200 hover:border-kku-300 hover:bg-kku-50 text-slate-600 transition-colors"
                >
                  {qp.label}
                </button>
              ))}
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl kku-gradient hover:opacity-95 text-white text-sm font-heading font-bold shadow-md flex items-center space-x-2 disabled:opacity-50 transition-all cursor-pointer shrink-0 ml-2"
            >
              {isAnalyzing ? (
                <>
        ...
        {errorMessage && (
          <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-800 text-sm flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                {activeStep > 1 && (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg border border-slate-300 hover:bg-slate-50 text-sm font-semibold text-slate-700"
                  >
                    ย้อนกลับ
                  </button>
                )}
                ...
                {activeStep < 6 ? (
                  <button
                    onClick={() => setActiveStep((prev) => prev + 1)}
                    className="px-4 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold flex items-center space-x-1"
                  >
                    <span>ถัดไป</span>
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className="px-5 py-2 rounded-lg kku-gradient text-white text-sm font-heading font-bold shadow-md flex items-center space-x-1.5 hover:opacity-95"
                  >
                    {isSaving ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (

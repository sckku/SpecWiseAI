...
<head>
  <meta charset="UTF-8">
  <title>แบบคำของบประมาณเงินรายได้ (งบลงทุน/ครุภัณฑ์) มหาวิทยาลัยขอนแก่น - ${proposal.code}</title>
  <style>
    @page {
      size: A4;
...
<body>
  <div class="no-print" style="margin-bottom: 20px; text-align: right;">
    <button onclick="window.print()" style="padding: 10px 20px; background: #8c2d19; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: bold;">🖨️ พิมพ์เอกสาร / บันทึกเป็น PDF</button>
  </div>

  <div class="header">
    <h1>แบบคำของบประมาณเงินรายได้ (งบลงทุน/ครุภัณฑ์) ประจำปีงบประมาณ พ.ศ. ${proposal.fiscalYear}</h1>
    <h2>มหาวิทยาลัยขอนแก่น (Khon Kaen University)</h2>
    <div class="badge">รหัสคำขอ: ${proposal.code} | สถานะ: ${proposal.status}</div>
  </div>
  ...
  <table>
    <tr>
      ...
      <td width="75%">${basic?.agency || `${proposal.faculty} / ${proposal.department}`}</td>
    </tr>
    <tr>
      <td class="field-label">ชื่อรายการครุภัณฑ์:</td>
      <td><strong>${basic?.itemName || proposal.title}</strong></td>
    </tr>
    <tr>
      <td class="field-label">จำนวนและวงเงิน:</td>
      <td>จำนวน ${basic?.quantity || proposal.quantity} ${basic?.unit || proposal.unit} | ราคาต่อหน่วย ${Number(basic?.unitPriceBaht || proposal.unitPriceBaht).toLocaleString()} บาท | <strong>วงเงินรวม ${Number(basic?.budgetBaht || proposal.totalBudgetBaht).toLocaleString()} บาท</strong></td>
    </tr>
    <tr>
      <td class="field-label">แผนงาน / แผนงานย่อย:</td>
      <td>${basic?.plan || "แผนงานจัดการศึกษาระดับอุดมศึกษา"} &gt; ${basic?.subPlan || "แผนงานย่อยส่งเสริมการวิจัยและนวัตกรรม"}</td>
    </tr>
    <tr>
      <td class="field-label">โครงการ:</td>
      <td>${basic?.project || "โครงการพัฒนาโครงสร้างพื้นฐานดิจิทัล"}</td>
    </tr>
    <tr>
      <td class="field-label">ประเภทการจัดหา:</td>
      <td>${basic?.procurementType || "จัดซื้อใหม่"}</td>
    </tr>
    <tr>
      <td class="field-label">ความสอดคล้อง S-Curve / SDGs:</td>
      <td>${basic?.sCurve || "-"} | SDGs: ${basic?.sdgs?.join(", ") || "-"}</td>
    </tr>
  </table>
  ...
  <table>
    <tr>
      ...
      <td width="75%">${basic?.objective || "-"}</td>
    </tr>
    <tr>
      <td class="field-label">เหตุผลความจำเป็น:</td>
      <td>${necessity?.details || "-"}</td>
    </tr>
    <tr>
      <td class="field-label">สถานที่ติดตั้งใช้งาน:</td>
      <td>${necessity?.installationLocation || "-"}</td>
    </tr>
    <tr>
      <td class="field-label">หลักสูตร/กลุ่มเป้าหมาย:</td>
      <td>${necessity?.targetCurriculum || "-"} (ผู้ใช้งานประมาณ ${necessity?.userCount || 0} คน)</td>
    </tr>
    <tr>
      <td class="field-label">ผลกระทบหากไม่ได้รับงบ:</td>
      <td>${necessity?.impactIfNotFunded || "-"}</td>
    </tr>
  </table>
  ...
  <table>
    ...
    <tr>
      <td>1</td>
      <td>${quotes?.vendor1?.name || "บริษัท ผู้ประกอบการรายที่ 1 จำกัด"}</td>
      <td>${Number(quotes?.vendor1?.price || 0).toLocaleString()}</td>
      <td>${quotes?.selectedVendorIndex === 1 ? "⭐ ราคาต่ำสุดที่เลือกใช้" : "-"}</td>
    </tr>
    <tr>
      <td>2</td>
      <td>${quotes?.vendor2?.name || "บริษัท ผู้ประกอบการรายที่ 2 จำกัด"}</td>
      <td>${Number(quotes?.vendor2?.price || 0).toLocaleString()}</td>
      <td>${quotes?.selectedVendorIndex === 2 ? "⭐ ราคาต่ำสุดที่เลือกใช้" : "-"}</td>
    </tr>
    <tr>
      <td>3</td>
      <td>${quotes?.vendor3?.name || "บริษัท ผู้ประกอบการรายที่ 3 จำกัด"}</td>
      <td>${Number(quotes?.vendor3?.price || 0).toLocaleString()}</td>
      <td>${quotes?.selectedVendorIndex === 3 ? "⭐ ราคาต่ำสุดที่เลือกใช้" : "-"}</td>
    </tr>
  </table>
  <div style="font-size: 11pt; color: #4b5563; margin-top: -8px; margin-bottom: 10px;">${quotes?.notes || ""}</div>
  ...
  <table>
    <tr>
      ...
      <td width="75%">${std?.ref1 || "เกณฑ์ราคากลางกระทรวงดิจิทัลฯ / สำนักงบประมาณ 2569"}</td>
    </tr>
    <tr>
  ...
  <table>
    <tr>
      ...
      <td width="75%">${readiness?.procurementReadiness || "พร้อมจัดทำร่าง TOR ทันที"}</td>
    </tr>
    <tr>
      <td class="field-label">ระยะเวลาลงนามสัญญา:</td>
      <td>${readiness?.contractSigning || "ภายใน 30 วัน"}</td>
    </tr>
    <tr>
      <td class="field-label">การส่งมอบและติดตั้ง:</td>
      <td>${readiness?.installationDelivery || "ภายใน 45 วัน"}</td>
    </tr>
  </table>
  ...
  <table>
    <tr>
      ...
      <td width="75%">${specSummary?.generalSpec || "-"}</td>
    </tr>
    <tr>
      <td class="field-label">สรุปสเปกทางเทคนิค:</td>
      <td>${specSummary?.technicalSpec || "-"}</td>
    </tr>
    <tr>
      <td class="field-label">ข้อมูลเพิ่มเติม:</td>
      <td>${form?.section8AdditionalNotes || "สอดคล้องตามแผนยุทธศาสตร์ มข."}</td>
    </tr>
  </table>
  ...
  <div class="signature-box">
    <div class="sign-col">
      ...
      <div class="sign-line">(${proposal.requesterName})</div>
